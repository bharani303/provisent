    ...
    return (
        ...
        >
            ...
            <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
                <div className="organic-blob absolute top-1/4 -left-32 w-96 h-96 bg-cyan-500/10 dark:bg-cyan-600/20 rounded-full blur-[120px] mix-blend-multiply dark:mix-blend-screen opacity-60"></div>
                <div className="organic-blob-reverse absolute bottom-1/4 -right-32 w-[500px] h-[500px] bg-purple-500/10 dark:bg-purple-600/20 rounded-full blur-[120px] mix-blend-multiply dark:mix-blend-screen opacity-60"></div>
            </div>

            {/* 3D Background */}
            <div className="absolute inset-0 z-0 hero-bg-blur opacity-20 dark:opacity-40 pointer-events-none">
            ...
            <div className="container relative z-10 px-6 mx-auto flex flex-col items-center justify-center text-center mt-20 pointer-events-auto">
                ...
                {/* Subtitle */}
                <p className="hero-subtitle text-lg md:text-xl text-foreground/50 max-w-3xl mb-12 font-light leading-relaxed">
                    {SITE_DATA.hero.subtitle}
                </p>

                {/* Stats */}
                <div className="hero-cta w-full max-w-4xl mx-auto flex flex-col md:flex-row items-center justify-center gap-8 md:gap-0 mt-8 mb-16 border-y md:border-y-0 border-border py-8 md:py-0">
                    <div className="flex flex-col items-center flex-1 w-full relative">
                        <span className="stat-number text-5xl md:text-4xl font-black text-foreground leading-none" data-target="50" data-suffix="K+">0</span>
                        <span className="text-[10px] md:text-xs uppercase tracking-widest text-foreground/30 mt-2 font-bold">Students</span>
                        {/* Desktop divider */}
                        <div className="hidden md:block absolute right-0 top-1/2 -translate-y-1/2 w-[1px] h-12 bg-border"></div>
                        {/* Mobile divider */}
                        <div className="md:hidden absolute bottom-[-16px] left-1/2 -translate-x-1/2 w-24 h-[1px] bg-border"></div>
                    </div>

                    <div className="flex flex-col items-center flex-1 w-full relative">
                        <span className="stat-number text-5xl md:text-4xl font-black text-cyan-400 leading-none" data-target="120" data-suffix="+">0</span>
                        <span className="text-[10px] md:text-xs uppercase tracking-widest text-foreground/30 mt-2 font-bold">Countries</span>
                        {/* Desktop divider */}
                        <div className="hidden md:block absolute right-0 top-1/2 -translate-y-1/2 w-[1px] h-12 bg-border"></div>
                        {/* Mobile divider */}
                        <div className="md:hidden absolute bottom-[-16px] left-1/2 -translate-x-1/2 w-24 h-[1px] bg-border"></div>
                    ...
                    <div className="flex flex-col items-center flex-1 w-full">
                        ...
                        <span className="text-[10px] md:text-xs uppercase tracking-widest text-foreground/30 mt-2 font-bold">Success</span>
                    </div>
                </div>
...
const MagneticButton = ({ children, primary }) => {
    ...
    return (
        <button
            ...
            className={`relative inline-flex items-center justify-center px-8 py-4 rounded-full font-bold transition-all duration-300 ${primary
                ? 'bg-foreground text-background hover:scale-105 shadow-premium active:scale-95'
                : 'bg-card-bg text-foreground border border-border shadow-premium backdrop-blur-glass hover:bg-background/80'
                }`}
        >
            {/* Magnetic wrapper */}
